...
#if defined(NDEBUG)
# error "Vbucks cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x36eccc16a31eda34e8f08b2d4ecfae60e56d0e3a9abfc1c9313750321d8361f0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vbucks: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vbucks Signed Message:\n";
...
{
    // Vbucks: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Vbucks: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Vbucks: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Vbucks: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vbucks: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vbucks: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xab7a98e9f48f8610109ff77d45ebd52b55b37e35991dd91ea839d1096e0c8257");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Facebook: Cambridge Analytica warning sentt to users";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ba4cd4ade9573c7ab17509639a446667e54e27c33f6ddf8e7edd67ac27c4465ca9c6c5216fa631944b7ea6bb3ef6b4c218c91dde1c76b7267e73c63214b45cf4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523304793;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085777033;

        if (fTestNet)
        {
            block.nTime    = 1523304773;
            block.nNonce   = 387027804;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6331cdf693a19c7546a826d7c626a31ccd087672273a49a35ea6995404f7d3ed"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xcd, 0xb4, 0xd8 }; // Vbucks: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VbucksMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VbucksMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VbucksMiner(CWallet *pwallet)
{
    printf("VbucksMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vbucks-miner");
    ...
    try { loop {
        ...
        printf("Running VbucksMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VbucksMiner, pwallet));
}
